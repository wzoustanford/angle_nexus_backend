...
# ========== CONCURRENCY ==========
# Hard-cap at 5 simultaneous workers
max_concurrent=6
...
echo "Launching up to $max_concurrent concurrent workers (of $total_workers total)" | tee -a "$log_file"
send_telegram "🔄 Worker job started at $(date)"
start_time=$(date +%s)

# ========== LAUNCH WORKERS WITH THROTTLING ==========
for i in $(seq 0 $(( total_workers - 1 ))); do
  echo "[$(date)] Starting worker $i/$((total_workers-1))..." | tee -a "$log_file"

  if [[ -n "$sample_count" ]]; then
    python3 test_build_dataset.py -wi "$i" -ns "$total_workers" -samples "$sample_count" \
      >> "$log_file" 2>&1 &
  else
    python3 test_build_dataset.py -wi "$i" -ns "$total_workers" \
      >> "$log_file" 2>&1 &
  fi

  # THROTTLE: if we already have $max_concurrent jobs running, wait for at least one to finish
